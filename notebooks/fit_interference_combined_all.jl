...
bkg_angle = best_model144.ϕB % 2π

# ╔═╡ 9f28f268-c44a-4992-a9e4-a0472d329989
bkg_angle_deg = (best_model144.ϕB % 2π)*180/π

# ╔═╡ df58764e-6ee5-48dd-86ec-cd4496cd5f57
let
	xlims = (specific_hist144m.binedges[1][1], specific_hist144m.binedges[1][end])
	plot()
	scatter!(bincenters(specific_hist144m), specific_hist144m.bincounts, yerr=sqrt.(specific_hist144m.bincounts), ylim=(0,:auto))
	...
	plot!(xlims..., lw=2, label="tot bkg") do m
        intensity(best_model144m, m) - intensity(BWCoherentIncoherent144m(best_model144m; c=0, b=0), m)
    end
	...
	vline!([4.461], color=:black, linestyle=:dash, lw=1, label="m = 4.461")
end
...
let
	...
	plot!(xlims..., lw=2, label="tot bkg") do m
        intensity(best_model108m, m) - intensity(BWCoherentIncoherent108m(best_model108m; c=0, b=0), m)
    end
	plot!(xlims..., lw=2, color=:red, label="inc bkg") do m
...
let
	...
	plot()
	scatter!(bincenters(specific_hist72m), specific_hist72m.bincounts, yerr=sqrt.(specific_hist72m.bincounts), ylim=(0,:auto))
	plot!(m->intensity(best_model72m, m), xlims..., lw=2)
	plot!(xlims..., fill=0, alpha=0.3, label="BW") do m
		intensity(BWCoherentIncoherent72m(best_model72m; c=0, b=0), m)
	end
	plot!(xlims..., lw=2, label="tot bkg") do m
        intensity(best_model72m, m) - intensity(BWCoherentIncoherent72m(best_model72m; c=0, b=0), m)
    end
	plot!(xlims..., lw=2, color=:red, label="inc bkg") do m
...
let
	...
	plot()
	scatter!(bincenters(specific_hist36m), specific_hist36m.bincounts, yerr=sqrt.(specific_hist36m.bincounts), ylim=(0,:auto))
	plot!(m->intensity(best_model36m, m), xlims..., lw=2)
	plot!(xlims..., fill=0, alpha=0.3, label="BW") do m
		intensity(BWCoherentIncoherent36m(best_model36m; c=0, b=0), m)
	end
	plot!(xlims..., lw=2, label="tot bkg") do m
        intensity(best_model36m, m) - intensity(BWCoherentIncoherent36m(best_model36m; c=0, b=0), m)
    end
	plot!(xlims..., lw=2, color=:red, label="inc bkg") do m
...
let
	...
	plot!(xlims..., lw=2, label="tot bkg") do m
        intensity(best_model0, m) - intensity(BWCoherentIncoherent0(best_model0; c=0, b=0), m)
    end
	plot!(xlims..., lw=2, color=:red, label="inc bkg") do m
...
let
	...
	plot!(xlims..., lw=2, label="tot bkg") do m
        intensity(best_model36, m) - intensity(BWCoherentIncoherent36(best_model36; c=0, b=0), m)
    end
	plot!(xlims..., lw=2, color=:red, label="inc bkg") do m
...
let
	...
	plot!(xlims..., lw=2, label="tot bkg") do m
        intensity(best_model72, m) - intensity(BWCoherentIncoherent72(best_model72; c=0, b=0), m)
    end
	plot!(xlims..., lw=2, color=:red, label="inc bkg") do m
...
let
	...
	plot()
	scatter!(bincenters(specific_hist108), specific_hist108.bincounts, yerr=sqrt.(specific_hist108.bincounts), ylim=(0,:auto))
	plot!(m->intensity(best_model108, m), xlims..., lw=2)
	plot!(xlims..., fill=0, alpha=0.3, label="BW") do m
		intensity(BWCoherentIncoherent108(best_model108; c=0, b=0), m)
	end
	plot!(xlims..., lw=2, label="tot bkg") do m
        intensity(best_model108, m) - intensity(BWCoherentIncoherent108(best_model108; c=0, b=0), m)
    end
	plot!(xlims..., lw=2, color=:red, label="inc bkg") do m
...
let
	...
	plot!(xlims..., lw=2, label="tot bkg") do m
        intensity(best_model144, m) - intensity(BWCoherentIncoherent144(best_model144; c=0, b=0), m)
    end
	plot!(xlims..., lw=2, color=:red, label="inc bkg") do m
...
let
	...
	plot!(xlims..., lw=2, label="tot bkg") do m
        intensity(best_model180, m) - intensity(BWCoherentIncoherent180(best_model180; c=0, b=0), m)
    end
	plot!(xlims..., lw=2, color=:red, label="inc bkg") do m
