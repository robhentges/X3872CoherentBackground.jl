...
# ╔═╡ 0effc528-445d-4b62-addd-4dcc99499347
values_list = ["144m", "108m", "72m", "36m", 0, 36, 72, 108, 144, 180]
...
# ╔═╡ f82c0b15-705b-44fa-a617-53d7744fb873
for val in values_list
    val_str = string(val)  # Convert to string
    struct_name = Symbol("BWCoherentIncoherent" * val_str)  # Struct name

    # Correct conversion of "m" values to negative numbers
    val_numeric = parse(Float64, occursin("m", val_str) ? "-" * replace(val_str, "m" => "") : val_str)

    @eval begin
        @with_kw struct $struct_name{T}
            a0::T  # norm of BW
            m0::T  # mass of BW
            Γ0::T  # width of BW
            ϕB::T  # phase of background
            c::T   # coherent background
            b::T   # incoh
            α::T   # bkg param
        end

        function intensity(model::$struct_name, m)
        ...
        function $struct_name(values::Vector)
            pars = NamedTuple{fieldnames($struct_name)}(values)
            $struct_name(; pars...)
        end
    end
end
